...
# Varying Eccentricity calculations
Ecc_size   = 10 
Ecc_vec      = np.linspace(0, .002, Ecc_size)
k2_vec = np.zeros(len(Ecc_vec))
E_vec = np.zeros(len(Ecc_vec))
...
# MPI 
num_per_rank = Ecc_size//size

Ecc_lower_bound = rank*num_per_rank 
Ecc_upper_bound = (rank + 1)*num_per_rank

print("Processor ", rank, ": iterations ", Ecc_lower_bound," to ", Ecc_upper_bound - 1, flush=True)

comm.Barrier() # start parallel processes

start_time = time.time()

for ind in range(Ecc_lower_bound, Ecc_upper_bound):
...
if rank == 0:
    k2_global = np.zeros(len(Ecc_vec))
    E_global = np.zeros(len(Ecc_vec))
...
if rank ==0:
    ...
    for ind in range(size*num_per_rank, Ecc_size):
    ...
    dill.dump([Ecc_vec, k2_global, E_global], file=open('./k2Q_N2vec_L{}_N{}{}_H{}.pickle'.format(np.max(L), N, label, int(H/1e5)), 'wb') )
